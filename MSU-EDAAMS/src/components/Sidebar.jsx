...
const Sidebar = () => {
  ...
  return (
    <div className="flex flex-col bg-[#0C356A] duration-500 justify-evenly">
      <div
        className={` bg-[#0C356A] h-screen ${open ? "w-56" : "w-14"
          } duration-500 text-white px-2`}
      >
        <div className="py-4 flex justify-end">
          <h1
            className={`${!open && "hidden"
              } mx-auto whitespace-pre font-semibold text-lg 0.5s ease-in-out`}
          >
            MSU EDAAMS
          </h1>
          <TiThMenu
        ...
        <div className="mt-4 flex flex-col gap-4 relative">
          {menus?.map((menu, i) => (
            <Link
              to={menu?.link}
              key={i}
              className="group flex items-center text-sm gap-3.5 fbg-[#23074d]ont-medium p-2 hover:border rounded-lg"
            >
              ...
              <Typography
                className={`${open && "hidden"
                  } absolute  text-sm left-24 bg-[#512B81] whitespace-pre text-white rounded-md drop-shadow-lg px-0 py-0 w-0 overflow-hidden group-hover:px-2 group-hover:py-1 group-hover:left-14 group-hover:duration-300 group-hover:w-fit`}
              >
                {menu?.name}
              </Typography>
            </Link>
      ...
      <div className="flex flex-col bg-[#0C356A] text-white items-center duration-500 p-2">
        ...
        <div
          className={`flex flex-col items-center whitespace-pre p-2 ${open ? "relative" : "inherent"
            }`}
        >
          <Typography
            className={`flex text-base font-semibold justify-center ${!open && " hidden"
              } ease-in-out`}
          >
            {currentUser}
          </Typography>
          <Typography
            className={`flex text-sm font-normal justify-center ${!open && "hidden"
              } ease-in-out`}
          >
            {userDesignation}
          </Typography>
